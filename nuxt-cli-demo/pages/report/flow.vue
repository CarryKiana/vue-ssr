<template>
  <div class="flowreport-page">
    <div class="top">
      <div class="top-title">
        <div class="left">流量报表</div>
        <range-picker class="right" v-model="dateTime" @change="dateChange"></range-picker>
      </div>
      <el-tabs class="tab" v-model="panpelType" @tab-click="tabChange">
        <el-tab-pane
          v-for="(pane,index) in panpelArr"
          :key="index"
          :label="pane.label"
          :name="pane.value"
        ></el-tab-pane>
    ...
    <div class="data-table">
      ...
      <div class="table-handle">
        <el-select
          ...
          :placeholder="panpelType=='adx'?'请选择adx':'请选择流量类型'"
      ...
      <div class="gcloud-table">
        <el-table :data="tableList" stripe @sort-change="tableSortChange">
          <el-table-column prop="groupName" label="ADX" key="adx" v-if="panpelType=='adx'"></el-table-column>
          <el-table-column prop="groupName" label="流量类型" key="flowType" v-else></el-table-column>
...
<script>
import rangePicker from "@/components/timerangepicker";
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query.msg == 1) {
      this.$DataMessage("导出数据失败");
      this.$router.push({ path: this.$route.path });
    } else if (this.$route.query.msg == 2) {
      this.$DataMessage("没有数据");
      this.$router.push({ path: this.$route.path });
    }
  },
  methods: {
    ...
    getOfflineReportChartData() {
      ...
      this.mapQuery.groupType = this.panpelType;
    ...
    getOfflineReportData() {
      ...
      this.tableQuery.groupType = this.panpelType;
